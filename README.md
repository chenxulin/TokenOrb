...
5. 悬浮球中的双层水波会随剩余额度改变水位并持续流动：大于 20% 为绿色，大于 10% 且小于等于 20% 为橙色，小于等于 10% 为红色。

## 跟随 Codex 运行

- 默认启用“跟随 Codex 启动/关闭”。应用会为当前 Windows 用户登记名为 `Token Orb` 的启动项。
- Windows 登录后会保留一个低频的托盘监测器；检测到 Codex 桌面窗口时启动悬浮球和实时接口，Codex 窗口关闭后约 2 秒内退出悬浮球及实时接口进程。
- 通知区域中的 Token Orb 图标可控制悬浮球：双击图标可手动显示；右键菜单中的“显示/隐藏悬浮球”用于切换可见状态，前面的“√”表示悬浮球当前可见；选择“退出”会同时结束悬浮球和本次后台监测器会话。
- “隐藏悬浮球”只隐藏界面，不结束后台 UI 进程，因此可以随时再次选择“显示悬浮球”恢复；在 Codex 运行期间手动隐藏后，它不会被后台轮询立即重新显示，Codex 下次启动时会恢复自动显示。
...
- 推送 `v*` 标签（例如 `v1.1`）会同时创建 GitHub Release 并附加 EXE、MSI、ZIP、源码包、说明和校验文件。
