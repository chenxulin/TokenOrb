...
- `TokenOrb.exe`：单文件便携版。
- `TokenOrb.msi`：Windows Installer 安装包，安装到 `%LOCALAPPDATA%\Programs\Token Orb`，并创建开始菜单入口；已安装旧版时会自动关闭并替换旧版。
...
如果已安装 Token Orb，直接双击新版 MSI 即可：安装器会关闭正在运行的 Token Orb、自动卸载旧版，再继续安装新版，无需先到“已安装的应用”手动卸载。同版本的更新构建也支持直接替换。

可从 Windows“已安装的应用”中卸载 Token Orb。升级或卸载时会移除旧程序文件、开始菜单入口和由 MSI 创建的启动项；用户的外观、位置、跟随偏好和错误日志会保留。
...
5. 悬浮球最外层轮廓使用淡蓝色（`#7DD3FC`），默认 60 px 球体的轮廓约为 1.8 px，并以约 3 秒周期进行柔和的亮度与透明度呼吸；球体以约 5.6 秒周期进行清晰但舒缓的柔和光斑、明暗变化和 96%–104% 呼吸缩放，光斑始终位于数字上方，百分比文字、窗口位置和点击区域保持固定。双层水波会随剩余额度改变水位并持续流动，内部进度环与水波始终使用相同警示色：大于 20% 为绿色，大于 10% 且小于等于 20% 为橙色，小于等于 10% 且大于 0% 为红色；0% 时最外层轮廓与 `0%` 数字使用相同的低额度红色，球体其他部分保持正常，内部不显示水波。正数低额度会保留最小可见波高，确保橙色和红色警告仍清晰可见；百分比文字始终显示真实额度。
...
- 显示“本地”：实时查询连续失败 3 次或接口断开时的降级状态；查询失败会按 2、4、8、16、30 秒指数退避自动重试，任意一次成功后清零。右键选择“立即刷新”可立即重试。
- 实时接口错误日志：`%LOCALAPPDATA%\Token Orb\realtime-errors.log`；日志超过 1 MB 后轮转为 `realtime-errors.previous.log`。
