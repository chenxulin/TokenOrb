...
final class AppLogger {
    ...
    private let maximumLifecycleBytes: UInt64 = 1_024 * 1_024
    ...
    func lifecycle(operation: String, details: String) {
        let logDirectory = directory
        let maximumBytes = maximumLifecycleBytes
        queue.async {
            do {
                try FileManager.default.createDirectory(
                    at: logDirectory,
                ...
                let current = logDirectory.appendingPathComponent("lifecycle.log")
                let previous = logDirectory.appendingPathComponent("lifecycle.previous.log")
                let attributes = try? FileManager.default.attributesOfItem(atPath: current.path)
                let size = (attributes?[.size] as? NSNumber)?.uint64Value ?? 0
                if size >= maximumBytes {
                    try? FileManager.default.removeItem(at: previous)
